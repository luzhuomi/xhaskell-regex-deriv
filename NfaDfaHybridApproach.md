...
Hybrid approach using a mix of DFA and NFA
where unintersting locations are 'cut off'.
...
>   pretty (Cut l rs) = P.brackets $ foldl (\a -> \r -> a P.<+> (P.text "+") P.<+> (pretty r)) (pretty $ head rs) (tail rs)
...
> posix w (Cut l rs) = 
>   case (filter ([] /=) $ map (posix w) rs) of
>     (m:_) -> [(l,range w)] -- we don't carry about anything below
>     []    -> []
> posix _ _ = []
...
> partDeriv :: Char -> Re l -> [Re l]
> partDeriv _ Phi = []
> partDeriv _ (Eps _) = []
> partDeriv c (Ch l c') 
>    | c == c'   = [Eps l]
>    | otherwise = []
> partDeriv c (Choice _ rs) =
>    nub $ concat $ map (partDeriv c) rs
> partDeriv c (Pair l r1 r2)
>   | containsEps r1 = nub $
>        [ Pair l r1' r2 | r1' <- partDeriv c r1 ]
>        ++ partDeriv c r2
>   | otherwise = nub [ Pair l r1' r2 | r1' <- partDeriv c r1 ]
> partDeriv c (Star l r) =
>      [ Pair l r' (Star l r) | r' <- partDeriv c r ]
> partDeriv c (Cut _ rs) =
>    nub $ concat $ map (partDeriv c) rs
>    -- only put here for completeness
>    -- we generally assume that there's no cut below another cut

> allPartDerivs :: Re l -> [Re l]
> allPartDerivs r =
>   let go rs = 
>        if (nub $ rs ++ next) == rs
>        then rs else go $ nub $ rs ++ next
>        where
>        next = nub $ rs ++ (concat $ [ partDeriv c r | 
>                             c <- sigma r, r <- rs ])
>   in go [r]

> -- plain simplification
> simpC :: Re l -> Re l
> simpC (Pair _ (Eps _) r) 
>  | isPhi r   = Phi
>  | otherwise = r
> simpC (Pair l r1 r2)
>   | isPhi r1 || isPhi r2 = Phi
>   | otherwise            = Pair l (simpC r1) (simpC r2)
> simpC (Choice l []) = Eps l
> simpC (Choice l [r]) = r
> simpC (Choice l rs)
>  | any isChoice rs =  
>     Choice l $ 
...
>                _            -> rs ++ [r])
>              [] rs
>  | otherwise = Choice l $ nub $ filter (not.isPhi) $ map simpC rs
> simpC (Star l1 (Eps l2)) = Eps l2
> simpC (Star l1 (Star l2 r)) = Star l2 r 
> simpC (Star l r)
>  | isPhi r   = Eps l
>  | otherwise = Star l $ simpC r 
> simpC (Cut l []) = Eps l
> simpC x = x 
...
> containsEps (Cut _ rs) = or $ map containsEps rs
...
> mDeriv _ Phi = Phi
> mDeriv c (Cut l rs) = 
>   Cut (mCh c l) $ nub $ concat $ map (partDeriv (snd c)) rs
...
> isPhi Eps{} = False
> isPhi Phi{} = True
> isPhi (Cut _ rs) = and $ map isPhi rs
...
> shift l1 Phi = Phi
> shift l1 (Cut l2 rs) = Cut (combine l1 l2) rs
...
> simp (Cut l []) = Eps l
> simp (Cut l [Eps _]) = Eps l
> simp (Cut l rs) = Cut l $ nub $ filter (not.isPhi) $ map simpC rs
> simp x = x 
...
> conv (Eps l) = Eps $ v l
> conv Phi = Phi
> conv (Cut l rs) = Cut (v l) $ map conv rs
...
> allDerivsN :: Int -> Re Int -> [Re Match]
> allDerivsN n r =
>   let go rs = 
>        if ((nub $ rs ++ next) == rs) || (length rs > n)
>        then rs else go $ nub $ rs ++ next
>        where
>        next = nub $ rs ++ [ simpFix $ mDeriv (1,c) r | 
>                             c <- sigma r, r <- rs ]
...
> repeatR n r
>   | n == 1 = r
>   | n == 2 = Pair dontCareLoc_ r r
>   | n > 2 = Pair dontCareLoc_ r $ repeatR (n-1) r

> myRange (i,j) r
>   | i < j && 0 < i = Choice dontCareLoc_ $ 
>                        map (\i-> repeatR i r) [i..j]

> optional r = Choice dontCareLoc_ [Eps dontCareLoc_, r]
...
*Main> length $ allDerivs (rWithCut 4)
43

*Main> length $ allDerivs (rWithCut 5) 
77


> -- abstraction notation: <r> don't care about submatches within r
> -- <.*><X(.?){1,n}><.*>
> rWithCut n = distinct $
>          Pair 1 (Cut 1 [Star 1 anyEx])
>            (Pair 1 
>               (Cut 1 [Pair 1 
>               (Ch 2 'X')
>               (Pair 3 (myRange (1,n) $ 
...
*Main> length $ allDerivs (rWithCuta 4)                                                      
16
*Main> length $ allDerivs (rWithCuta 5)
18
...
*Main> length $ allDerivs (r2NoCut 4)  
123

> r2WithCut n = distinct $ 
>      Pair 1 
>           (Cut 1
>             [(Star 1 $
>               (Pair 3 (myRange (1,n) $ 
...
*Main> length $ allDerivs (r2WithCut 4) 
14
